...
const BEATS = {
  full: { loaderOut: 0.45, lights: 0.5, neon: 0.75, person: 1.0, eyebrow: 1.2, line: 1.35, pre: 1.6, name: 1.75, buttons: 2.2, captions: 2.6, loops: 2.8, typeStep: 0.03 },
  short: { loaderOut: 0, lights: 0, neon: 0.1, person: 0, eyebrow: 0.1, line: 0.15, pre: 0.2, name: 0.25, buttons: 0.4, captions: 0.55, loops: 0.8, typeStep: 0 },
};

const EYEBROW = "PEOPLE X IDEAS X SOLUTIONS";
const PERSON_MASK = "radial-gradient(ellipse 42% 27% at 50% 48%, #000 55%, transparent 100%)";
...
export function Hero() {
  ...
  const at = (beat: keyof typeof BEATS.full, duration: number, extra?: Transition): Transition =>
    mode === "none"
      ? { duration: 0 }
      : { delay: beats[beat], duration: mode === "short" ? Math.min(duration, 0.5) : duration, ease: EASE, ...extra };
  ...
  const photoY = useTransform(scrollYProgress, [0, 1], still ? ["0%", "0%"] : ["0%", "16%"]);
  const contentOpacity = useTransform(scrollYProgress, [0, 0.55], still ? [1, 1] : [1, 0]);
  const contentY = useTransform(scrollYProgress, [0, 0.55], still ? [0, 0] : [0, -40]);
  const darken = useTransform(scrollYProgress, [0, 0.8], still ? [0, 0] : [0, 0.6]);
  ...
  useEffect(() => {
    ...
    animateNeon(neonScope.current, { opacity: [0, 0.9, 0.25, 1] }, { delay: beats.neon, duration: 0.55 }).then(() => {
      if (cancelled || !neonScope.current) return;
      loop = animateNeon(
        neonScope.current,
        { opacity: [1, 0.78, 1, 0.8, 1, 0.35, 1, 0.55, 1] },
        { duration: 7, times: [0, 0.2, 0.4, 0.6, 0.86, 0.88, 0.9, 0.92, 1], repeat: Infinity, ease: "easeInOut" }
      );
    });
    return () => {
      cancelled = true;
  ...
  const lights =
    mode === "full"
      ? { filter: ["brightness(0.1)", "brightness(0.1)", "brightness(0.75)", "brightness(0.3)", "brightness(1)"] }
      : { filter: "brightness(1)" };
  ...
  return (
    ...
    >
      ...
      <motion.div
        aria-hidden
        initial={{ opacity: 1 }}
        animate={ready ? { opacity: 0, transitionEnd: { display: "none" } } : undefined}
        transition={mode === "full" ? { delay: beats.loaderOut, duration: 0.3, ease: "easeOut" } : { duration: 0 }}
      ...
      >
        <div className="absolute inset-x-0 top-[8.75%] flex flex-col items-center md:static md:items-start">
          <motion.p
            ...
            variants={{
              hidden: {},
              show: {
                transition: still ? {} : { delayChildren: beats.eyebrow, staggerChildren: beats.typeStep },
              },
            }}
          ...
          >
            {EYEBROW.split("").map((ch, i) => (
              <motion.span
                ...
                variants={{
                  hidden: { opacity: 0 },
                  show: { opacity: 1, transition: { duration: still ? 0 : mode === "short" ? 0.3 : 0.05 } },
                }}
                className={ch === "X" ? "mx-[0.55em] text-[#e9b863]" : undefined}
              >
                {ch}
              </motion.span>
            ))}
        ...
        <motion.h1
          ...
          animate={ready ? { opacity: 1, clipPath: "inset(-20% -5% -20% -5%)" } : undefined}
        ...
        <div className="absolute inset-x-[14.6%] top-[68.06%] grid h-[9.14vw] grid-cols-2 gap-[2vw] md:static md:mt-12 md:h-16 md:w-[460px] md:gap-4">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={ready ? { opacity: 1, y: 0 } : undefined} transition={at("buttons", 0.7)}>
            <TalkLink
            ...
            >
              <span aria-hidden className="btn-sweep pointer-events-none absolute inset-y-0 left-0 w-1/3 bg-[linear-gradient(90deg,transparent,rgba(255,255,255,0.6),transparent)]" />
              <Send className="relative h-[3.6vw] w-[3.6vw] md:h-5 md:w-5" strokeWidth={2.4} />
          ...
          <motion.div
            ...
            animate={ready ? { opacity: 1, y: 0 } : undefined}
            transition={at("buttons", 0.7, { delay: mode === "none" ? 0 : beats.buttons + 0.12 })}
          >
            <Link
              href="/#services"
      ...
      <motion.div style={{ y: photoY }} className="absolute inset-0 overflow-hidden md:relative">
        <motion.div style={{ x: tiltX, y: tiltY }} className="absolute inset-0">
          <motion.div
            initial={{ scale: 1.02 }}
            animate={ready && !still ? { scale: [1.02, 1.07] } : { scale: 1.02 }}
            transition={
              ready && !still
                ? { delay: beats.loops, duration: 22, repeat: Infinity, repeatType: "mirror", ease: "easeInOut" }
                : { duration: 0 }
            }
            className="absolute inset-x-0 top-0 aspect-[941/1672]"
          >
            <motion.div
              ...
              transition={
                mode === "full"
                  ? { delay: beats.lights, duration: 0.9, times: [0, 0.15, 0.3, 0.42, 1], ease: "easeOut" }
                  : at("lights", 0.6)
              }
            ...
            >
              ...
              <motion.div
                ...
                transition={at("person", 1.5, { ease: "easeInOut" })}
                className="absolute inset-0"
            ...
            <div
              ...
              aria-hidden
              style={{ opacity: 0 }}
              className="absolute left-[76%] top-[22%] h-[24%] w-[30%] rounded-full bg-[radial-gradient(closest-side,rgba(255,178,84,0.5),rgba(255,150,60,0.18)_55%,transparent)] mix-blend-screen"
            />
          </motion.div>
        </motion.div>
        <div
        ...
        />
        <motion.div aria-hidden style={{ opacity: darken }} className="absolute inset-0 bg-black" />
      </motion.div>
      ...
      <motion.p
        ...
        initial={{ opacity: 0, letterSpacing: "0.6em" }}
        animate={ready ? { opacity: 1, letterSpacing: "0.28em" } : undefined}
        transition={at("captions", 1.1, { delay: mode === "none" ? 0 : beats.captions + 0.12 })}
